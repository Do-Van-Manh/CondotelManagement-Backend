# Script to reorganize test cases CSV file
...
try {
    ...
    $testCases = Import-Csv -Path $inputFile -Encoding UTF8
    
    Write-Host "Đã đọc $($testCases.Count) test cases"
    
    # Process each test case
    $processedCount = 0
    foreach ($tc in $testCases) {
        $oldFunc = $tc.'Function Name'
        $oldSheet = $tc.'Sheet Name'
        
        $newFunc = Normalize-FunctionName $oldFunc
        $newSheet = Normalize-SheetName $oldSheet
        
        $tc.'Function Name' = $newFunc
        $tc.'Sheet Name' = $newSheet
        
        $processedCount++
    }
    
    Write-Host "Đã xử lý $processedCount test cases"
    Write-Host "Đang ghi file $outputFile..."
    
    # Export to CSV
    $testCases | Export-Csv -Path $outputFile -Encoding UTF8 -NoTypeInformation
    
    # Statistics
    $sheetStats = $testCases | Group-Object 'Sheet Name' | Select-Object Name, Count | Sort-Object Name
    $funcStats = $testCases | Group-Object 'Function Name' | Select-Object Name, Count | Sort-Object Count -Descending | Select-Object -First 20
    ...
    Write-Host "`nHoàn thành! Đã tạo file $outputFile"
} catch {
    Write-Host "Lỗi: $_" -ForegroundColor Red
    Write-Host "Đang thử phương pháp thay thế..."
    ...
    # Replace function names (more complex, need to be careful)
    foreach ($oldFunc in $functionMapping.Keys) {
        $newFunc = $functionMapping[$oldFunc]
        # Replace in CSV format: ,"Function Name",
        $content = $content -replace ",`"$oldFunc`",", ",`"$newFunc`","
    }
    
    $content | Out-File -FilePath $outputFile -Encoding UTF8 -NoNewline
    Write-Host "Đã tạo file $outputFile bằng phương pháp thay thế"
}
